# RUN-005: Codex Launch Brief for GT-005
...
Выполнить `GT-005` для `Codex` внутри `runtimes/codex/` так, чтобы при stress-сценарии система использовала bounded retry, а затем честно эскалировала, если локального repair path недостаточно.
...
В рамках `RUN-005 / GT-005` `Codex` должен:

...
Но зрелая конструкция требует не только коммуникации, а ещё и bounded failure governance.

Иначе организация формально "живёт", но фактически может бесконечно переотправлять один и тот же сбойный шаг.

Поэтому в проектной конструкции observer/control layer обязан поддерживать ещё и такие инварианты:

- retry budget на уровень директивы;
- явное состояние `retry_budget_exhausted`;
- терминальные исходы `human_review_required` и `wave_failed`;
- сохранение `partial_success`, если peer-runtime успешно завершил свою часть wave;
- запрет бесконечного redispatch под видом нормальной автономной работы.
...
На зрелом уровне здесь должны отражаться не только happy-path статусы, но и bounded failure states:

- текущий retry counter;
- исчерпан ли retry budget;
- локальная ветка в `partial_success`, `human_review_required` или `wave_failed`;
- ждёт ли ветка peer-closeout или уже заблокировала всю wave.
...
В зрелой организации сюда должны попадать не только сами сбои, но и типовые failure-governance сценарии:

- repeated relaunch без полезного прогресса;
- `retry_budget_exhausted`;
- `partial_success` при провале соседней ветки;
- ложный terminal success из-за рассинхрона `directive` и `ack`;
- необходимость переводить wave в `human_review_required` или `wave_failed`.

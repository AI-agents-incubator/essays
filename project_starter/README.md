...
3. starter kit заранее добавляет базовую git-защиту для `.claude/settings.local.json`;
4. пользователь заполняет один верхнеуровневый входной артефакт;
5. runtime читает bootstrap и intake;
6. после этого агентная организация сама строит:
   - уточняющие вопросы;
   - product brief;
   - engineering spec;
   - task graph;
...
При этом starter kit теперь включает и минимальный security baseline:

- локальный permission-файл `.claude/settings.local.json` заранее защищается от коммита;
- это делается как часть проектного scaffold, а не остаётся надеждой на локальные привычки пользователя.
...
- базовая защита `.claude/settings.local.json` от git присутствует по умолчанию.
